...
def color_negative_red(value):
    ...
    """
    return ['background-color: #00ff0020',]*len(value) if value['change'] > 0 else ['background-color: #ff000020']*len(value)
...
def main():
    ...
    try:
        ...
        with col1:
            col1.metric("Advance", data['advance_decline']['advance'], border=True,)
        with col2:
            col2.metric("Decline", data['advance_decline']['decline'], border=True,)
        with col3:
            col3.metric("Unchanged", data['advance_decline']['unchanged'], border=True,)
        st.dataframe(data['data_frame'].style.apply(color_negative_red, axis=1),height=600, column_config={
            ...
            "totalTurnover": st.column_config.NumberColumn("Value (₹ Crores)", format="%0.2f"),
            "marketCap": st.column_config.NumberColumn("FFMC (₹ Crores)", format="%0.2f"),
