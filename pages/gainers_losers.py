...
with tab1:
    try:
        ...
        if gainers is None:
            ...
            filtered_data = [
                {k: d.get(k) for k in keys_to_extract}
                for d in gainers[index[selectBox]]["data"]
            ]
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "open_price": st.column_config.NumberColumn(
                            "Open Price", format="%0.2f"
                        ),
                        "high_price": st.column_config.NumberColumn(
                            "High Price", format="%0.2f"
                        ),
                        "low_price": st.column_config.NumberColumn(
                            "Low Price", format="%0.2f"
                        ),
                        "ltp": st.column_config.NumberColumn("LTP", format="%0.2f"),
                        "prev_price": st.column_config.NumberColumn(
                            "Previous Price", format="%0.2f"
                        ),
                        "perChange": st.column_config.NumberColumn(
                            "% Change", format="%0.2f%%"
                        ),
...
with tab2:
    try:
        ...
        if loosers is None:
            ...
            filtered_data = [
                {k: d.get(k) for k in keys_to_extract}
                for d in loosers[index[selectBox]]["data"]
            ]
            df = pd.DataFrame(filtered_data)
            if df.empty:
                raise Exception("No data available for loosers.")
