...
def main():
    ...
    show_row_color = st.toggle("Show Row Color", label_visibility="visible", value=False)
    try:
        ...
        st.dataframe(df, height=600, column_config={
            ...
            "trdVal": st.column_config.NumberColumn("Value (₹ Crores)", format="%0.2f"),
            ...
            "perChange30d": st.column_config.NumberColumn("30d % Change", format="%0.2f%%"),
            "perChange365d": st.column_config.NumberColumn("365d % Change", format="%0.2f%%"),
        }, hide_index=True)
