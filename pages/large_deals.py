...
tab1, tab2, tab3 = st.tabs(
    ["Bulk Deals", "Block Deals", "Short Selling"], width="stretch"
)

with tab1:
    try:
        st.dataframe(
            nse.large_deals("BULK_DEALS_DATA"),
            height=600,
            column_config={
                ...
                "watp": st.column_config.NumberColumn(
                    "Trade Price/ Weighted. AVG. Pprice"
                ),
            },
        )
        st.warning(
...
with tab2:
    try:
        st.dataframe(
            nse.large_deals("BLOCK_DEALS_DATA"),
            height=600,
            column_config={
                "date": st.column_config.DateColumn("Date"),
                "symbol": st.column_config.TextColumn("Symbol"),
...
with tab3:
    try:
        st.dataframe(
            nse.large_deals("SHORT_DEALS_DATA"),
            height=600,
            column_config={
                "date": st.column_config.DateColumn("Date"),
                "symbol": st.column_config.TextColumn("Symbol"),
